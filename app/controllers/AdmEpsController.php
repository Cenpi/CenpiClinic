...
class AdmEpsController extends ControllerBase
{
    ...
    public function editAction($idEps)
    {
        if (!$this->request->isPost()) {
            ...
            $this->view->idEps = $adm_ep->idEps;

            $this->tag->setDefault("idEps", $adm_ep->idEps);
            $this->tag->setDefault("nitEps", $adm_ep->nitEps);
            $this->tag->setDefault("nombreEps", $adm_ep->nombreEps);
